...
def plot_labeled_image(binary_data): 
    labels=measure.label(binary_data,connectivity=2)  #8 connectivity labeled
    dst=color.label2rgb(labels)  #display different arear with different color
    print('regions number:',labels.max()+1)  #dispaly the number of connectivity blocks

    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(8, 4))
    ax1.imshow(binary_data, plt.get_cmap('gray'), interpolation='nearest')
    ax1.axis('off')
