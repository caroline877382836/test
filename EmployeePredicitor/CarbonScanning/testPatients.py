...
@author: CNXuaCar
This doc is used to test the whole patient cases in the json file
"""
import numpy as np
import os
import sys
...
def Tranformkeyw(cName):
    tName=''
    if cName=='nominalEnergy':
        tName='Energy'
    elif cName=='nKernelGernerationPrecision':
        tName='KernelPrecision'
    elif cName=='applyNuclearScatteringCorrection':
        tName='Correction'
    elif cName=='nFinalGernerationPrecision':
        tName='FinalPrecision'
    elif cName.find('isocenterPos')>=0:
        tName='isocenterPos'
    elif cName.find('voxelSize')>=0:
        tName='voxelSize'
    else:
        tName=cName
    return tName
...
def plot_delta_sigma(del_sigma): 
    ...
    plt.figure()
    plt.plot(del_sigma_X,'r--', label='del_sigma_X')
    plt.plot(del_sigma_Y,'g--', label='del_sigma_Y')        
    #plt.plot(IDD_test, 'bo',label = "IDD_test",IDD_BenchMark,'g^',label = "IDD_Uppsala")  # 'r--','bs','g^': red dashes, blue squares and green triangles
...
def plot_idd_3(IDD_test,IDD_BenchMark,idd): 
    plt.figure()
    plt.plot(IDD_test,'r--', label='IDD_test')
    plt.plot(IDD_BenchMark,'g--', label='IDD_BenchMark') 
    plt.plot(idd,'b--', label='IDD') 
    #plt.plot(IDD_test, 'bo',label = "IDD_test",IDD_BenchMark,'g^',label = "IDD_BenchMark")  # 'r--','bs','g^': red dashes, blue squares and green triangles
    plt.legend(loc='best', numpoints=1, handlelength=0) 
...
if __name__ == "__main__" :  
    ...
    # change the json file to your local path
...
summary_sheet,sum_raw_idx=book.add_sum_sheet("summary")
font_color=book.change_font('red')
font=book.format()

summary_sheet.write(sum_raw_idx, 0, "No.",font) 
summary_sheet.write(sum_raw_idx, 1, "Testing Purpose",font) 
summary_sheet.write(sum_raw_idx, 2, "Change Condition",font) 
summary_sheet.write(sum_raw_idx, 4, "Sigma",font)
summary_sheet.write(sum_raw_idx, 5, "IDD",font)  # bench - Proton
summary_sheet.write(sum_raw_idx, 6, "Lateral 1D Profile",font)
summary_sheet.write(sum_raw_idx, 7, "Lateral 2D Profile",font)
summary_sheet.write(sum_raw_idx, 8, "3D dose matrix",font)
summary_sheet.write(sum_raw_idx, 9, "Gamma Index",font)
sum_raw_idx=sum_raw_idx+1
summary_sheet.write(sum_raw_idx, 0, "0",font)
sum_raw_idx=sum_raw_idx+1
...
for i in range(len(init_paras.Jason_data._patients_config_lst)):
    ...
    try:
        ...
        data_case0={}
        data_case0=init_paras.Jason_data.patient_cases_dict["0"]
        if(patient_case_name=="0"):
            ...
            if(data_case0["BenchMark_exe_name"]==patient_config_lst._BenchMark_exe_name):
                sheet_raw_idx=book.write(sheet_raw_idx,0,"BenchMark_exe",  \
                                os.path.join(init_paras.Jason_data._benchMark_exe_root, patient_config_lst._BenchMark_exe_name))
            else:
                sheet_raw_idx=book.write_color(sheet_raw_idx,0,"BenchMarch_exe",  \
                                os.path.join(init_paras.Jason_data._benchMark_exe_root, patient_config_lst._BenchMark_exe_name))
            if(data_case0["test_exe_name"]==patient_config_lst._test_exe_name):
                sheet_raw_idx=book.write(sheet_raw_idx,0,"test_exe", \
                                os.path.join(init_paras.Jason_data._test_exe_root, patient_config_lst._test_exe_name))
            else:
                sheet_raw_idx=book.write_color(sheet_raw_idx,0,"test_exe", \
                                os.path.join(init_paras.Jason_data._test_exe_root, patient_config_lst._test_exe_name))
            if(data_case0["ED_file_name"]==patient_config_lst._ED_file_name):
                sheet_raw_idx=book.write(sheet_raw_idx,0,"ED_file_name", \
                                os.path.join(init_paras.Jason_data._ED_temp_dir, patient_config_lst._ED_file_name))
            else:
                change_content.append("ED")
                change_content.append(str(patient_config_lst._ED_file_name))
                sheet_raw_idx=book.write_color(sheet_raw_idx,0,"ED_file_name", \
                                os.path.join(init_paras.Jason_data._ED_temp_dir, patient_config_lst._ED_file_name))
...
# write sigma statistics
    ...
    if np.amax(data_test.Data3D) == 0.0:
        continue
    # image_raw_idx = book.insert_image2Excel(image_raw_idx, 0, "plot delta_sigma", 
                                            # os.path.join(os.path.join(init_paras.output_path + '\\' + patient_case_name,"delta_sigma.png"))) 

    
        # plot_idd(IDD_test, os.path.join(init_paras.output_path + '\\' + patient_case_name,"IDD_test.png"))
        # image_raw_idx = book.insert_image2Excel(image_raw_idx, 0, "plot IDD_test", 
                                                # os.path.join(init_paras.output_path + '\\' + patient_case_name,"IDD_test.png"))
    if 'UPPSALA' in benchMark_path:
...
# Generate a 3D point grid from the total size [mm] and the number of voxels.
    ...
    testPurpose=[]
    for linekey in range(0,len(change_content),2):
        testPurpose.append(Tranformkeyw(change_content[linekey]))


    link='internal:'+str(patient_case_name)+'!A1'
    if(patient_case_name=='0'):
        summary_sheet.write_url(2,0,link,string=patient_case_name)
        summary_sheet.write(2,4,float(Analysis_results["Delta Sigma Analysis Result"]["percentage_err(%):"]))
        summary_sheet.write(2,5,float(Analysis_results["IDD Analysis Result"]["percentage_err(%):"]))
        summary_sheet.write(2,6,float(Analysis_results["Lateral 1D Profile Analysis Result"]["percentage_err(%):"]))
        summary_sheet.write(2,7,float(Analysis_results["Lateral 2D Profile Analysis Result"]["percentage_err(%):"]))
        summary_sheet.write(2,8,float(Analysis_results["3D Dose Matrix Analysis"]["percentage_err(%):"]))
        summary_sheet.write(2,9,float(Analysis_results["Gamma Index Analysis Result"]["percentage_passing:"]))
        continue
    summary_sheet.write_url(3+number_change,0,link,string=patient_case_name)
    summary_sheet.write(3+number_change,4,float(Analysis_results["Delta Sigma Analysis Result"]["percentage_err(%):"]))
    summary_sheet.write(3+number_change,5,float(Analysis_results["IDD Analysis Result"]["percentage_err(%):"]))
    summary_sheet.write(3+number_change,6,float(Analysis_results["Lateral 1D Profile Analysis Result"]["percentage_err(%):"]))
    summary_sheet.write(3+number_change,7,float(Analysis_results["Lateral 2D Profile Analysis Result"]["percentage_err(%):"]))
    summary_sheet.write(3+number_change,8,float(Analysis_results["3D Dose Matrix Analysis"]["percentage_err(%):"]))
    summary_sheet.write(3+number_change,9,float(Analysis_results["Gamma Index Analysis Result"]["percentage_passing:"]))
    test_compare=[]
    for line in range(0,len(testPurpose),1):
        if not testPurpose[line] in test_compare:
            summary_sheet.write(3+number_change+line,1,testPurpose[line])
            summary_sheet.write(3+number_change+line,2,change_content[2*line]+'='+change_content[2*line+1])
            test_compare.append(testPurpose[line])
        else:
            summary_sheet.write(3+number_change+line,2,change_content[2*line]+'='+change_content[2*line+1])
    if(len(testPurpose))>0:
        number_change=number_change+len(testPurpose)
    else:
